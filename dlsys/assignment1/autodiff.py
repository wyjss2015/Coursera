...
def gradients(output_node, node_list):
    ...
    for node in reverse_topo_order:
        ...
        grad_to_inputs = node.op.gradient(node, node_to_output_grad[node])
        if grad_to_inputs is not None:
            for nd, grad in zip(node.inputs, grad_to_inputs):
                if nd not in node_to_output_grads_list:
                    node_to_output_grads_list[nd] = [grad]
                else:
                    node_to_output_grads_list[nd] += [grad]
